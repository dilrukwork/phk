"""Build an epub from a reviewed OCR markdown file.

Usage:
    python build_epub.py <name> [--start-line N] [--end-line N]
                          [--range START:END ...]
                          [--suffix TEXT] [--label TEXT]
                          [--toc-limit N] [--toc-keep I,J,K]
                          [--cover-image] [--page-numbers]

    Reads src/ocr/<name>-ocr.md, applies corrections from
    src/ocr/myedit.md, strips resolved [[?word]] glossary markers, and
    writes src/published/<name><suffix>.epub with the Sinhala font and
    stylesheet embedded.

    --start-line/--end-line (1-indexed, inclusive) restrict the build to
    a single slice of the source file. Left out, the whole reviewed body
    is used (front-matter comment header and the trailing "REVIEW NOTES"
    comment block are stripped automatically either way).

    --range START:END (repeatable) stitches together several
    non-contiguous slices in the order given -- e.g. front matter plus
    one talk, skipping the talks in between. Overrides --start-line/
    --end-line when given.

    --toc-limit trims the in-text <div class="toc-list"> to its first N
    <div class="toc-entry"> rows. --toc-keep instead keeps an explicit,
    1-based list of entry positions (e.g. "1,2,3,4,6" to keep the 4
    front-matter entries plus Talk 2's, skipping Talk 1's) -- use this
    whenever --range excludes an earlier talk. --toc-keep takes
    precedence over --toc-limit when both are given.

    --label is appended to the book title in parentheses, e.g.
    "Talk 2 preview".

    --cover-image rasterizes the booktitle/booksubtitle/bookauthor divs
    from the first chunk into a JPEG (via Pillow + the embedded Sinhala
    font) and registers it as the epub's official cover image/metadata.
    Kindle's library thumbnail renderer doesn't apply the embedded font
    to the text-based cover page, so without this the thumbnail shows
    as blank/tofu for Sinhala titles.

    --page-numbers appends a small sequential "page N" marker to every
    page-break-delimited chunk (1-indexed across the whole build, not
    tied to the original PDF's printed page numbers).

    Any <div class="dhammacakka">...</div> placeholder in the source is
    replaced with the Dhamma wheel image from src/resources/dhammawheel.png,
    embedded and resized to DHAMMAWHEEL_MAX_DIM.

OCR and epub publishing are separate steps: this script does not touch
the -ocr.md source, so it's safe to re-run after further manual edits.
"""
...
DHAMMAWHEEL_MAX_DIM = 300  # px -- displayed at a few em, no need for the 1024px source
...
def load_myedit(name: str) -> list[tuple[str, str]]:
    ...
    for line in myedit_path.read_text(encoding="utf-8").splitlines():
        ...
        if match:
            ...
            print(f"myedit.md: could not parse line, skipping: {stripped!r}")
    return corrections
...
# Sinhala combining marks (vowel signs, virama) and the zero-width joiner used
# for conjuncts are punctuation-adjacent in Unicode's general category, so
# Python's \w does not treat them as word characters even though they're
# fused onto the preceding consonant as part of the same word. Widen the
# "word character" set to the whole Sinhala block (+ ZWJ, + \w) so a
# whole-word check doesn't mistake a mid-word cluster boundary for a real
# word boundary.
...
def build_epub(name: str, start_line: int | None, end_line: int | None, ranges: list[str] | None,
                suffix: str, label: str, toc_limit: int | None, toc_keep: list[int] | None,
                cover_image: bool = False, page_numbers: bool = False) -> None:
    src_path = OCR_DIR / f"{name}-ocr.md"
    if not src_path.exists():
        raise FileNotFoundError(f"No such OCR file: {src_path}")
    ...
    if cover_image:
        ...
        if title_m and subtitle_m and author_m:
            ...
            print("--cover-image requested but booktitle/booksubtitle/bookauthor divs not found in this slice; skipping")

    chunks = split_chapters(content)
    if not chunks:
        raise ValueError("No content left after slicing -- check --start-line/--end-line")
    ...
    out_path = PUBLISHED_DIR / f"{name}{suffix}.epub"
    epub.write_epub(out_path, book)

    print(f"{name}: {len(chunks)} section(s) -> {out_path} "
          f"({out_path.stat().st_size / 1024:.1f} KB)")
    print(f"Corrections applied ({len(applied)} of {len(corrections)} myedit.md entries matched):")
    for old, new, count in applied:
        print(f"  {old!r} -> {new!r}  x{count}")
    unmatched = [old for old, _ in corrections if old not in [a[0] for a in applied]]
    if unmatched:
        print(f"  (not found in this slice, left for a later batch: {unmatched})")
    print(f"[[?...]] markers resolved to plain text ({len(stripped_markers)}):")
    for entry in stripped_markers:
        print(f"  {entry}")
    if dhammawheel_count:
        print(f"Dhamma wheel image embedded ({dhammawheel_count} placeholder(s) replaced)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--range", action="append", default=None, help="START:END, repeatable")
